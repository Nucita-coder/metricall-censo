...
- **Estilo Uniforme de Badges / Etiquetas (Sin Emojis y Sin Redundancias)**: Queda prohibido el uso de emojis en etiquetas o estatus (`PAGO PROCESADO`, `PAGO EN REVISIÓN`, etc.). Se prohíbe colocar etiquetas redundantes como `WS`, `REPORTE FALLA` o `GESTIÓN ONLINE` en las cabeceras de las tarjetas en Gestión Online / WhatsApp Bot. Las tarjetas solo llevarán badge superior cuando tengan un servicio contratado (`HOGAR`, `PYMES`, `DEDICADO`, `ISP`), un estatus de pago (`PAGO PROCESADO`, `PAGO RECHAZADO`, `PAGO EN REVISIÓN`) o en tableros de cobranza (`COBRANZA`). Las cabeceras en Ventas Online y Reporte Falla permanecen completamente limpias.
