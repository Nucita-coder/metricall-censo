...
import { Send, LifeBuoy, User, MessageSquare, ArrowLeft, Image as ImageIcon, X } from 'lucide-react-native';
import * as ImagePicker from 'expo-image-picker';
import { useAuth } from '../../context/AuthContext';
import { supabase } from '../../lib/supabase';
import { uploadImageToSupabase } from '../../services/uploadImage';
import { styles } from './ModalSoporteTecnico.styles';
import { MensajeSoporte, UsuarioConversacion } from './types';

export function ChatSoporteHumano() {
  ...
  const cargarMensajesChat = async (userId: string, targetId: string) => {
    try {
      const { data, error } = await supabase
        .from('soporte_mensajes')
        .select('*')
        .or(
          `and(emisor_id.eq.${userId},receptor_id.eq.${targetId}),and(emisor_id.eq.${targetId},receptor_id.eq.${userId})`
        )
        .order('created_at', { ascending: true });

      if (error) throw error;
      setMensajes((data || []) as MensajeSoporte[]);

      await supabase
        .from('soporte_mensajes')
        .update({ leido: true })
        .eq('emisor_id', targetId)
        .eq('receptor_id', userId)
        .eq('leido', false);
    } catch (e: unknown) {
      console.error('Error al cargar mensajes de chat:', (e as Error).message);
    }
  };
  ...
  if (!soporteId) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.emptyTitle}>Sin Encargado Humano</Text>
        <Text style={styles.emptySub}>
          El Administrador aún no ha asignado un técnico de guardia en la empresa.{'\n\n'}
          Puedes consultar y resolver cualquier duda con el Asistente IA en la pestaña superior.
        </Text>
      </View>
    );
  }

  if (isEsSoporte && !chatActivoUserId) {
    return (
      <ScrollView style={styles.chatBody} contentContainerStyle={{ padding: 16 }}>
        <Text style={styles.sectionTitle}>Bandeja de Consultas ({conversaciones.length})</Text>
        {conversaciones.length === 0 ? (
          ...
          conversaciones.map((conv) => (
            <TouchableOpacity
              ...
              onPress={() => {
                setChatActivoUserId(conv.usuario_id);
                setChatActivoNombre(conv.nombre_completo);
                if (currentUserId) cargarMensajesChat(currentUserId, conv.usuario_id);
              }}
            >
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={styles.convUltimo} numberOfLines={1}>
                  {conv.ultimo_mensaje?.startsWith('[IMG]') ? '📷 Imagen adjunta' : conv.ultimo_mensaje}
                </Text>
              </View>
  ...
  return (
    <View style={{ flex: 1 }}>
      {isEsSoporte && chatActivoUserId && (
        <View style={styles.chatSubHeader}>
          <TouchableOpacity onPress={() => setChatActivoUserId(null)} style={{ marginRight: 10 }}>
            <ArrowLeft size={18} color="#B6C2CF" />
          </TouchableOpacity>
          <Text style={styles.chatSubTitle}>{chatActivoNombre}</Text>
        </View>
      )}
      ...
      >
        {mensajes.length === 0 ? (
          <Text style={styles.emptySubCenter}>
            {isEsSoporte
              ? `Inicia la conversación con ${chatActivoNombre}`
              : `Chat directo con ${soporteNombre}. Escribe tu duda técnica.`}
          </Text>
        ) : (
          (mensajes || [])
            .filter((m) => m && typeof m === 'object' && m.mensaje)
            .map((m, idx) => {
      ...
      <View style={styles.inputFooter}>
        ...
        <TouchableOpacity
          style={[styles.btnSend, (!nuevoMensaje.trim() || sending) && { opacity: 0.5 }]}
